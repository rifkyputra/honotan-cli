...
import type { ArchitecturePattern, ClientArchitecturePattern, Framework, ApiFramework, ClientFramework, InboundAdapter, OutboundAdapter, ExistingResourceInfo, InfraPackage } from '../types';
  ...
  const { action } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: 'Add adapter(s) to an existing resource', value: 'add-adapter' },
      ],
    },
  ]);
  return action;
  ...
  const { architecture } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: 'Clean Architecture (Layer-based)', value: 'clean-architecture' },
      ],
    },
  ]);
  return architecture;
  ...
  const { name } = await inquirer.prompt([
    {
      ...
      validate: (val: string) => val.trim().length > 0 || 'Resource name is required',
    },
  ]);
  return name.trim();
}

export async function promptInboundAdapters(framework: Framework): Promise<InboundAdapter[]> {
  ...
  const { adapters } = await inquirer.prompt([
    {
      ...
      choices: [
        { name: `HTTP (${frameworkLabel} controller + routes + validation)`, value: 'http' as const, checked: true },
        { name: 'WebSocket (generic WebSocket adapter)', value: 'websocket' as const },
      ],
      validate: (val: unknown[]) => val.length > 0 || 'Select at least one inbound adapter',
    },
  ]);
  return adapters;
}
  ...
  const { adapters } = await inquirer.prompt([
    {
      ...
      choices: [
        { name: 'In-Memory Repository', value: 'in-memory' as const, checked: true },
        { name: 'Database (Bun.sql for Postgres)', value: 'database' as const },
        { name: 'Cache (Bun.redis)', value: 'cache' as const },
      ],
      validate: (val: unknown[]) => val.length > 0 || 'Select at least one outbound adapter',
    },
  ]);
  return adapters;
}
...
export async function promptExistingResource(resources: ExistingResourceInfo[]): Promise<ExistingResourceInfo> {
...
export async function promptAdapterDirection(): Promise<'inbound' | 'outbound'> {
...
export async function promptNewInboundAdapters(existing: ExistingResourceInfo, framework: Framework): Promise<InboundAdapter[]> {
  const frameworkLabel = framework.charAt(0).toUpperCase() + framework.slice(1);
  const choices: { name: string; value: InboundAdapter }[] = [];
  if (!existing.hasHttp) {
    choices.push({ name: `HTTP (${frameworkLabel} controller + routes + validation)`, value: 'http' });
  }
  if (!existing.hasWebSocket) {
    choices.push({ name: 'WebSocket (generic WebSocket adapter)', value: 'websocket' });
  }
  ...
  const { adapters } = await inquirer.prompt([
    {
      ...
      choices,
      validate: (val: unknown[]) => val.length > 0 || 'Select at least one adapter',
    },
  ]);
  return adapters;
}

export async function promptNewOutboundAdapters(existing: ExistingResourceInfo): Promise<OutboundAdapter[]> {
  ...
  if (!existing.hasDatabase) {
    choices.push({ name: 'Database (Bun.sql for Postgres)', value: 'database' });
  }
  ...
  const { packages } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: 'Event-Driven (RabbitMQ via amqplib)', value: 'event-driven' as const },
      ],
    },
  ]);
  return packages;
