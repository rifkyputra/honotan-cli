...
export function generateDockerComposeDev(data: MonorepoTemplateData): string {
  ...
  if (data.hasDb) {
    ...
    container_name: ${data.kebabName}-postgres-dev
  ...
  if (data.hasCache) {
    ...
    container_name: ${data.kebabName}-redis-dev
    restart: unless-stopped
    ports:
      - "6379:6379"
    healthcheck:
  ...
  if (data.hasEventDriven) {
    ...
    container_name: ${data.kebabName}-rabbitmq-dev
    restart: unless-stopped
    environment:
  ...
  const serverService = `  server:
    build:
      ...
      target: deps
    container_name: ${data.kebabName}-server-dev
    restart: unless-stopped
    ports:
      - "3000:3000"
    volumes:
