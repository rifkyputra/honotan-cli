...
export function generateVsInMemoryRepositoryTemplate(data: TemplateData): string {
  const { capitalizedName, camelCaseName, pluralName, name } = data;

  return `import type { ${capitalizedName}RepositoryPort } from './${name}.repository';
import type { ${capitalizedName}Entity, Create${capitalizedName}Data, Update${capitalizedName}Data } from './${name}.entity';
