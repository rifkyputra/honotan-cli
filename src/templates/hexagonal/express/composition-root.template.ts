...
export function generateExpressCompositionRootTemplate(data: TemplateData): string {
  ...
  if (hasHttp) {
    wiringLines.push(`export const ${camelCaseName}Routes = create${capitalizedName}Routes(${camelCaseName}UseCases);`);
  }
  if (hasWebSocket) {
